...
CREATE TABLE `poll_choices` (
  ...
  CONSTRAINT `poll_choices_FK_question_id` FOREIGN KEY (`question_id`) REFERENCES `poll_questions` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `poll_questions` (
  ...
  CONSTRAINT `poll_questions_FK_poll_id` FOREIGN KEY (`poll_id`) REFERENCES `polls` (`id`)
...
CREATE TABLE `poll_responses` (
  ...
  KEY `poll_responses_FK_respondent_user_id` (`respondent_id`),
  CONSTRAINT `poll_responses_FK_choices` FOREIGN KEY (`choice_id`) REFERENCES `poll_choices` (`id`),
  CONSTRAINT `poll_responses_FK_respondent_user_id` FOREIGN KEY (`respondent_id`) REFERENCES `users` (`id`)
...
CREATE TABLE `polls` (
  ...
  CONSTRAINT `polls_FK_user` FOREIGN KEY (`owner_id`) REFERENCES `users` (`id`)
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`innotech`@`%` PROCEDURE `create_poll`(owner_id INT(10)UNSIGNED, title_input VARCHAR(500), category_input VARCHAR(100), poll_expiry_input DATE, invite_only_input BOOL, limits_on_input_input BOOL)
    MODIFIES SQL DATA
BEGIN
	INSERT INTO polls (title, category, owner_id, poll_expiry, invite_only, limits_on_input)
	VALUES(title_input, category_input, owner_id, poll_expiry_input, invite_only_input, limits_on_input_input);
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`innotech`@`%` PROCEDURE `get_polls_by_owner`(owner_id_input VARCHAR(100))
BEGIN
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`innotech`@`%` PROCEDURE `user_response`(respondent_id_input INT(10)UNSIGNED, choice_id_input INT(10)UNSIGNED)
    MODIFIES SQL DATA
BEGIN
	INSERT INTO poll_responses (choice_id, respondent_id)
		VALUES (choice_id_input, respondent_id_input);
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`innotech`@`%` PROCEDURE `user_signup`(username_input VARCHAR(100), password_input VARCHAR(100), email_input VARCHAR(100), dob_input DATE, token_input VARCHAR(45), first_name_input VARCHAR(100), last_name_input VARCHAR(100))
    MODIFIES SQL DATA
BEGIN
	IF dob_input IS NULL THEN
	...
	ELSE
		INSERT INTO users(username, password, email, dob, first_name, last_name)
		VALUES(username_input, password_input, email_input, dob_input, first_name_input, last_name_input);
	SET @new_user_id = LAST_INSERT_ID();
	INSERT INTO user_session (token, user_id)
		VALUES (token_input, @new_user_id);
	SELECT user_id, token
...
-- Dump completed on 2023-03-17  8:30:24
